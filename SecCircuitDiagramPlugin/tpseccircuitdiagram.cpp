    ...
    m_state(1)
{
    ...
    connect(timer, &QTimer::timeout, this, &TPSecCircuitDiagram::on_timer_timeout);
    //有闭合回路才开启定时器
    if(m_state == 1 || m_state == 2 || (m_heaterContactorClosed && m_heaterFaultContactorClosed))
    {
        timer->start();
    }
...
int TPSecCircuitDiagram::chargeLevel() const {
    return m_chargeLevel;
}

void TPSecCircuitDiagram::setChargeLevel(int level) {
...
bool TPSecCircuitDiagram::mainContactorClosed() const {
    return m_mainContactorClosed;
}

void TPSecCircuitDiagram::setMainContactorClosed(bool closed) {
...
bool TPSecCircuitDiagram::dischargeContactorClosed() const {
    return m_dischargeContactorClosed;
}

void TPSecCircuitDiagram::setDischargeContactorClosed(bool closed) {
...
bool TPSecCircuitDiagram::chargeContactorClosed() const {
    return m_chargeContactorClosed;
}

void TPSecCircuitDiagram::setChargeContactorClosed(bool closed) {
...
bool TPSecCircuitDiagram::heaterFaultContactorClosed() const {
    return m_heaterFaultContactorClosed;
}

void TPSecCircuitDiagram::setHeaterFaultContactorClosed(bool closed) {
...
bool TPSecCircuitDiagram::isHeating() const {
    return m_isHeating;
}

void TPSecCircuitDiagram::setIsHeating(bool heating) {
...
bool TPSecCircuitDiagram::heaterContactorClosed() const {
    return m_heaterContactorClosed;
}

void TPSecCircuitDiagram::setHeaterContactorClosed(bool closed) {
...
bool TPSecCircuitDiagram::limitedContactorClosed() const {
    return m_limitedContactorClosed;
}

void TPSecCircuitDiagram::setLimitedContactorClosed(bool closed) {
...
QColor TPSecCircuitDiagram::packColor1() const {
    return m_packColor1;
}

void TPSecCircuitDiagram::setPackColor1(const QColor &color) {
...
QColor TPSecCircuitDiagram::packColor2() const {
    return m_packColor2;
}

void TPSecCircuitDiagram::setPackColor2(const QColor &color) {
...
QColor TPSecCircuitDiagram::packColor3() const {
    return m_packColor3;
}

void TPSecCircuitDiagram::setPackColor3(const QColor &color) {
...
QColor TPSecCircuitDiagram::packColor4() const {
    return m_packColor4;
}

void TPSecCircuitDiagram::setPackColor4(const QColor &color) {
...
QColor TPSecCircuitDiagram::packColor5() const {
    return m_packColor5;
}

void TPSecCircuitDiagram::setPackColor5(const QColor &color) {
...
QColor TPSecCircuitDiagram::packColor6() const {
    return m_packColor6;
}

void TPSecCircuitDiagram::setPackColor6(const QColor &color) {
...
int TPSecCircuitDiagram::customLanguage() const
{
    return m_language;
}

void TPSecCircuitDiagram::setCustomLanguage(const int language) {
...
int TPSecCircuitDiagram::componentState() const
{
    return m_state;
}

void TPSecCircuitDiagram::setComponentState(const int state)
{
    if(m_state != state)
    {
...
int TPSecCircuitDiagram::showPack() const
{
    return m_showPack;
}

void TPSecCircuitDiagram::setShowPack(bool show)
{
    if(m_showPack != show)
    {
        m_showPack = show;
...
void TPSecCircuitDiagram::drawChargeLevel(QPainter &painter, const QRect &batteryRect) {
 ...
 * @brief TPSecCircuitDiagram::drawWireToMainContactor
 * @param painter
 * @param batteryX
 * @param batteryY
 * @param batteryWidth
...
void TPSecCircuitDiagram::on_timer_timeout()
{
...
void TPSecCircuitDiagram::drawMainContactor(QPainter &painter, int x, int y, int batteryWidth)
{
    int radius = 4;
 ...
 * 长度 width() * 3 / 10
 * @brief TPSecCircuitDiagram::drawWireToSystemVoltage
 * @param painter
 * @param mainContactorX
 * @param mainContactorY
 * @param batteryWidth
 */
void TPSecCircuitDiagram::drawWireToSystemVoltage(QPainter &painter, int mainContactorX, int mainContactorY, int batteryWidth)
{
 ...
 * 长度 height() / 12
 * 充电起点 width() * 11 / 120
 * @brief TPSecCircuitDiagram::drawWireToHeaterFaultContactor
 * @param painter
 * @param mainContactorX
 * @param mainContactorY
 * @param batteryWidth
 */
void TPSecCircuitDiagram::drawWireToHeaterFaultContactor(QPainter &painter, int mainContactorX, int mainContactorY, int batteryWidth)
{
    ...
    if(m_heaterContactorClosed && m_heaterFaultContactorClosed)
    {
        ...
        else
        {
            for(int i = 0; i < chargeHeatPositionList.size(); i++)
            {
                if(chargeHeatPositionList[i] <= -HALF_ENERGY_BLOCK_WIDTH || chargeHeatPositionList[i] >= width() * 11 / 120 + verticalLineLength - HALF_ENERGY_BLOCK_WIDTH)
                {
                    continue;
                }
                //能量冒头
                if(chargeHeatPositionList[i] < 0){
                    ...
                    int x2 = startX - width() * 11 / 120;
                    int x1 = startX - width() * 11 / 120 + chargeHeatPositionList[i] + ENERGY_BLOCK_WIDTH;
                    drawGradientLineSegment(x1, startY, x2, startY, Qt::red, painter, colorAt);
                }
                //能量完全在水平线上
                else if(chargeHeatPositionList[i] + ENERGY_BLOCK_WIDTH <= width() * 11 / 120)
                {
                    int x = startX - width() * 11 / 120 + chargeHeatPositionList[i];
                    drawGradientLineSegment(x, startY, x + ENERGY_BLOCK_WIDTH, startY, Qt::red, painter);
                }
                //能量在折线中
                else if(chargeHeatPositionList[i] < width() * 11 / 120)
                {
                    int x1 = startX - width() * 11 / 120 + chargeHeatPositionList[i];
                    int y3 = startY + chargeHeatPositionList[i] + ENERGY_BLOCK_WIDTH - width() * 11 / 120;
                    drawGradientPolylineSegment(x1, startY, startX, startY, startX, std::min(y3, startY + verticalLineLength), Qt::red, painter);
                }
                //能量完全在垂线中
                else if(chargeHeatPositionList[i] + ENERGY_BLOCK_WIDTH <= width() * 11 / 120 + verticalLineLength)
                {
                    drawGradientLineSegment(startX, startY + chargeHeatPositionList[i] - width() * 11 / 120, startX, startY + chargeHeatPositionList[i] - width() * 11 / 120 + ENERGY_BLOCK_WIDTH, Qt::red, painter);
                }
                //能量进入开关 有bug
                // else
                // {
...
void TPSecCircuitDiagram::drawHeaterFaultContactor(QPainter &painter, int x, int y, int batteryWidth)
{
    int radius = 4;
    int centerDistance = batteryWidth / 4; // 调整为电池宽度的1/4
 ...
 * @brief TPSecCircuitDiagram::drawWireToHeater
 * @param painter
 * @param heaterFaultContactorX
 * @param heaterFaultContactorY
 * @param batteryWidth
 */
void TPSecCircuitDiagram::drawWireToHeater(QPainter &painter, int heaterFaultContactorX, int heaterFaultContactorY, int batteryWidth)
{
...
void TPSecCircuitDiagram::drawHeater(QPainter &painter, int x, int y, int side)
{
    // 设置正方形边框画笔颜色和宽度
 ...
 * @brief TPSecCircuitDiagram::drawWireToHeaterContactor
 * @param painter
 * @param heaterX
 * @param heaterY
 * @param heaterWidth
 */
void TPSecCircuitDiagram::drawWireToHeaterContactor(QPainter &painter, int heaterX, int heaterY, int heaterWidth)
{
 ...
 * @brief TPSecCircuitDiagram::drawHeaterContactor
 * @param painter
 * @param x
 * @param y
 */
void TPSecCircuitDiagram::drawHeaterContactor(QPainter &painter, int x, int y)
{
    int radius = 4;
...
void TPSecCircuitDiagram::drawSystemVoltage(QPainter &painter, int startX, int startY)
{
    // 还原视口和窗口设置，以绘制其他部分
    painter.setViewport(0, 0, width(), height());
    painter.setWindow(0, 0, width(), height());
 ...
 * @brief TPSecCircuitDiagram::drawWireToDischargeContactor
 * @param painter
 * @param startX
 * @param startY
 */
void TPSecCircuitDiagram::drawWireToDischargeContactor(QPainter &painter, int startX, int startY)
{
    // 设置画笔颜色
    QPen pen(Qt::black);
    pen.setWidth(2);
...
void TPSecCircuitDiagram::drawDischargeContactor(QPainter &painter, int x, int y, int batteryWidth)
{
    int radius = 4;
 ...
 * 长度：width() / 6 + height() / 6
 * @brief TPSecCircuitDiagram::drawWireToLimitedContactor
 * @param painter
 ...
 * 长度：width() / 6 + height() / 6
 * @brief TPSecCircuitDiagram::drawLimitedContactor
 * @param painter
 * @param x
 * @param y
 */
void TPSecCircuitDiagram::drawLimitedContactor(QPainter &painter, int x, int y)
{
    int radius = 4;
    int centerDistance = width() / 20; // 调整为电池宽度的1/4
...
void TPSecCircuitDiagram::drawGradientLineSegment(int x1, int y1, int x2, int y2, Qt::GlobalColor edgeColor, QPainter &painter, double color2At)
{
    //绘制能量块覆盖的部分
...
void TPSecCircuitDiagram::drawGradientPolylineSegment(int x1, int y1, int x2, int y2, int x3, int y3, Qt::GlobalColor edgeColor, QPainter &painter)
{
    // 计算渐变起点和终点，跨越垂直和水平部分
...
void TPSecCircuitDiagram::buildEnergyPositionListWithLimit()
{
    energyPositionList.clear();
    energyPositionList = {
 ...
 * 长度：width() / 8 + width() / 24
 * @brief TPSecCircuitDiagram::drawWireToChargeContactor
 * @param painter
 * @param startX
 * @param startY
 * @param batteryWidth
 */
void TPSecCircuitDiagram::drawWireToChargeContactor(QPainter &painter, int startX, int startY, int batteryWidth)
{
    // 设置画笔颜色
    QPen pen(Qt::black); // 导线颜色
    pen.setWidth(5);
...
void TPSecCircuitDiagram::drawChargeContactor(QPainter &painter, int x, int y, int batteryWidth)
{
    int radius = 4;
    int centerDistance = batteryWidth / 4; // 调整为电池宽度的1/4
 ...
 * @brief TPSecCircuitDiagram::drawWireFromNegativeElectrode
 * @param painter
 * @param batteryX
 * @param batteryY
 * @param batteryWidth
 * @param batteryHeight
 */
void TPSecCircuitDiagram::drawWireFromNegativeElectrode(QPainter &painter, int batteryX, int batteryY, int batteryWidth, int batteryHeight)
{
    // 还原视口和窗口设置，以绘制其他部分
    painter.setViewport(0, 0, width(), height());
    painter.setWindow(0, 0, width(), height());
...
void TPSecCircuitDiagram::paintEvent(QPaintEvent *event) {
    Q_UNUSED(event);
...
TPSecCircuitDiagram::~TPSecCircuitDiagram()
{
    if (timer->isActive()) {
        timer->stop();
    }
