...
TNewBattery::TNewBattery(QWidget *parent)
    : QWidget(parent), m_chargeLevel(50) { // 初始化电量为50%
...
void TNewBattery::paintEvent(QPaintEvent *event) {
    ...
    font.setPointSize(width()/20); // 增加字体大小，例如16
    painter.setFont(font);

    // 电池主体
    QRect batteryRect(width() * 2 / 10, height() * 2 / 10, width() * 6 / 10, height() * 6 / 10); // 0.6宽高，两边各留0.2的空间
